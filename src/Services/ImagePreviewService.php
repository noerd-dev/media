...
class ImagePreviewService
{
    ...
    public function regenerateThumbnail(Media $media): ?string
    {
        ...
        if (in_array($extension, ['png', 'jpg', 'jpeg', 'webp'])) {
            ...
            $thumbPath = $this->paths->thumbnailDirectory((int) $media->tenant_id) . '/thumb_' . $randomName . '.jpg';
            Storage::disk($disk)->put($thumbPath, (string) $thumbnail->toJpeg());
        }

        if ($extension === 'pdf') {
            $randomName = Str::random();
            $thumbPath = $this->paths->thumbnailDirectory((int) $media->tenant_id) . '/pdf_' . $randomName . '.jpg';
            $fullPreviewPath = Storage::disk($disk)->path($thumbPath);

            Storage::disk($disk)->makeDirectory($this->paths->thumbnailDirectory((int) $media->tenant_id));
